...
export function PortfolioWeeklyChart({
  ...
  // ========== INVESTMENT TIMELINE CHART DATA ==========
  ...
  // Calculate investment chart points
  ...
  const interpolatedPoint = useMemo(() => {
    if (interpolatedX !== null && validInvestmentPoints.length > 0) {
      ...
      if (adjustedX <= validInvestmentPoints[0].x) {
        ...
        const lastPoint = validInvestmentPoints[validInvestmentPoints.length - 1];
        return {
          x: lastPoint.x,
          y: lastPoint.y,
          cumulativeAmount: lastPoint.cumulativeAmount,
          date: lastPoint.date,
          propertyTitle: lastPoint.propertyTitle,
                ...
                {isSelected && (
                  <View
                    style={{
                      position: 'absolute',
                      bottom: height + 8,
                      backgroundColor: isDarkColorScheme ? 'rgba(0, 0, 0, 0.9)' : 'rgba(0, 0, 0, 0.85)',
                  ...
                  >
                    ...
                    <View
                      style={{
                        ...
                        borderTopColor: isDarkColorScheme ? 'rgba(0, 0, 0, 0.9)' : 'rgba(0, 0, 0, 0.85)',
                      }}
                    />
                  </View>
                )}
                ...
                >
                  <View
                    style={{
                      ...
                      borderColor: isDarkColorScheme ? 'rgba(255, 255, 255, 0.3)' : 'rgba(255, 255, 255, 0.5)',
                    }}
                  />
                </TouchableOpacity>
              </View>
  ...
  // ========== RENDER INVESTMENT TIMELINE CHART ==========
  const renderInvestmentTimelineChart = () => {
    ...
    // FIXED: Update selected position with correct coordinate handling
          ...
          <Svg width={chartWidth} height={chartHeight}>
            <Defs>
              <LinearGradient id="investmentAreaGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                <Stop offset="0%" stopColor="rgba(213, 246, 92, 0.3)" stopOpacity="1" />
                <Stop offset="50%" stopColor="rgba(213, 246, 92, 0.15)" stopOpacity="1" />
                <Stop offset="100%" stopColor="rgba(213, 246, 92, 0.02)" stopOpacity="1" />
              </LinearGradient>
            </Defs>
            ...
            {dynamicAreaPath && <Path d={dynamicAreaPath} fill='rgba(197, 233, 35, 0.51)' />}
            ...
            {/* Selected point with glow */}
            {selectedPoint && (
              <>
                <Circle cx={selectedPoint.x} cy={selectedPoint.y} r="16" fill="rgba(205, 246, 92, 0.15)" />
                <Circle cx={selectedPoint.x} cy={selectedPoint.y} r="10" fill="rgba(205, 246, 92, 0.25)" />
                <Circle cx={selectedPoint.x} cy={selectedPoint.y} r="7" fill="rgb(244, 246, 92)" />
            ...
            {!isDragging && validPoints.map((point, index) => (
              <Circle
                ...
                cy={point.y}
                r="3.5"
                fill={selectedIndex === index ? "rgb(244, 246, 92)" : isDarkColorScheme ? '#1a1a1a' : '#ffffff'}
                stroke="rgb(218, 246, 92)"
                strokeWidth="2"
          ...
          {selectedPoint && (
            <View
              style={{
                ...
                backgroundColor: isDarkColorScheme ? 'rgba(20, 20, 20, 0.95)' : 'rgba(0, 0, 0, 0.9)',
                ...
                elevation: 8,
                borderWidth: 1,
                borderColor: isDarkColorScheme ? 'rgba(218, 246, 92, 0.3)' : 'rgba(255, 255, 255, 0.1)',
              }}
            >
              <Text style={{ color: '#FFFFFF', fontSize: 18, fontWeight: '700', marginBottom: 4, letterSpacing: 0.3 }}>
                ${selectedPoint.cumulativeAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </Text>
              <Text style={{ color: 'rgba(255, 255, 255, 0.7)', fontSize: 12, marginBottom: 2, fontWeight: '500' }}>
                {selectedPoint.label}
              </Text>
              <Text 
